...
{
    public class JobsRoutineFlow : IMachineJobImplementation
    {
        ...
        {
            return JobHelper.HandleRoutineJobInputOutput( Structure, Job, Logic );
        }
        ...
        public bool HandleJobDeletionLogic( MachineStructure Structure, MachineJob Job, JobDeletionLogic Logic, Action ActionOrNull, bool IsFromBlowingUp )
        {
            ...
            {
                ArcenDebugging.LogDebugStageWithStack( "JobsRoutineFlow HandleJobDeletionLogic error: " + Logic + " " + (Job?.ID ?? "[null]"), debugStage, e, Verbosity.ShowAsError );
                return false;
            }
        }
    }
