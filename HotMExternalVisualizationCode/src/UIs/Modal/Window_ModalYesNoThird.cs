...
{
    public class Window_ModalYesNoThird : ToggleableWindowController, IInputActionHandler
    {
        public static Window_ModalYesNoThird Instance;
        public Window_ModalYesNoThird()
        {
            Instance = this;
        ...
        public override bool GetShouldDrawThisFrame_Subclass()
        {
            ...
            if ( data == null )
                return false;
            if ( data.Style != ModalPopupStyle.YesNoThird )
                return false;
            return true;
        }
        ...
        public class bThird : ButtonAbstractBase
        {
            public override void GetTextToShowFromVolatile( ArcenDoubleCharacterBuffer Buffer )
            {
                Buffer.AddRaw( Engine_Universal.CurrentPopups.FirstOrDefault?.ThirdText ?? string.Empty, ColorTheme.GetBasicLightTextPurple( this.Element.LastHadMouseWithin ) );
            }
            public override MouseHandlingResult HandleClick_Subclass( MouseHandlingInput input )
            {
                try
                {
                    ...
                    try
                    {
                        popupData?.OnThird?.Invoke();
                    }
                    catch ( Exception e )
                    {
                        ArcenDebugging.LogSingleLine( "Error in clicking Third button: " + e, Verbosity.ShowAsError );
                    }
                }
                catch
                {
